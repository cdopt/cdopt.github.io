...
`CLASS grassmann_torch(var_shape, device = torch.device('cpu'), dtype = torch.float64)`
...
The variables in `grassmann_torch` is not restricted in 2D-tensors, they can be Nd-tensors whose `.size()` is `var_shape`. In that cases, for each index `(i,j,k,...)`, we have `torch.matmul( X(i,j,k,...,:,:).T ,X(i,j,k,...,:,:)) ` is a $p\times p$-dimensional identity matrix. 
